...
class Saha_Facebook_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'facebook-likebox',
			apply_filters( 'saha_widget_name', esc_html__( '&raquo; Facebook Like Box', 'saha' ) ),
			array(
				'classname' => 'widget_facebook_likebox',
				'description' => __( 'Display a Facebook Like Box to connect visitors to your Facebook Page', 'saha' )
			)
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( empty( $like_args['href'] ) || ! $this->is_valid_facebook_url( $like_args['href'] ) ) {
			if ( current_user_can('edit_theme_options') ) {
				echo $before_widget;
				echo '<p>' . sprintf( __( 'It looks like your Facebook URL is incorrectly configured. Please check it in your <a href="%s">widget settings</a>.', 'saha' ), admin_url( 'widgets.php' ) ) . '</p>';
				echo $after_widget;
			}
		...
		$like_bg_colour          = apply_filters( 'saha_fb_likebox_bg', ( 'dark' == $like_args['colorscheme'] ? '#000' : '#fff' ), $like_args['colorscheme'] );
		...
		if ( ! empty( $title ) ) :
			...
			echo apply_filters( 'saha_facebook_likebox_title', $likebox_widget_title, $title, $page_url );
		...
		do_action( 'saha_stats_extra', 'widget', 'facebook-likebox' );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'href' ); ?>">
				...
				<br />
				<small><?php _e( 'The Like Box only works with <a href="http://www.facebook.com/help/?faq=174987089221178">Facebook Pages</a>.', 'saha' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'colorscheme' ); ?>">
				<?php _e( 'Color Scheme', 'saha' ); ?>
				<select name="<?php echo $this->get_field_name( 'colorscheme' ); ?>" id="<?php echo $this->get_field_id( 'colorscheme' ); ?>">
					<option value="light" <?php selected( $like_args['colorscheme'], 'light' ); ?>><?php _e( 'Light', 'saha' ); ?></option>
					<option value="dark" <?php selected( $like_args['colorscheme'], 'dark' ); ?>><?php _e( 'Dark', 'saha' ); ?></option>
				</select>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_faces' ); ?>">
				...
				<?php _e( 'Show Faces', 'saha' ); ?>
				<br />
				<small><?php _e( 'Show profile photos in the plugin.', 'saha' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'stream' ); ?>">
				...
				<?php _e( 'Show Stream', 'saha' ); ?>
				<br />
				<small><?php _e( 'Show the profile stream for the public profile.', 'saha' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_border' ); ?>">
				...
				<?php _e( 'Show Border', 'saha' ); ?>
				<br />
				<small><?php _e( 'Show a border around the plugin.', 'saha' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'force_wall' ); ?>">
				...
				<?php _e( 'Show Wall', 'saha' ); ?>
				<br />
				<small><?php _e( 'Show the wall for a Places page rather than friend activity.', 'saha' ); ?></small>
			</label>
		</p>
	...
	function get_default_args() {
		...
		return apply_filters( 'saha_facebook_likebox_defaults', $defaults );
	}
