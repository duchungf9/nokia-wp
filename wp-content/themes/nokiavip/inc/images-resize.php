...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
	function saha_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {

		global $wpdb;

		if ( empty( $url ) )
			return new WP_Error( 'no_image_url', __( 'No image URL has been entered.','saha' ), $url );
		...
		$name = wp_basename( $file_path, ".$ext" );
	        ...
	        if ( 'bmp' == $ext ) {
			return new WP_Error( 'bmp_mime_type', __( 'Image is BMP. Please use either JPG or PNG.','saha' ), $url );
		}
	...
	function saha_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
		...
		if ( !file_exists( $dest_file_name ) ) {
			...
			if ( !$size )
				return new WP_Error( 'file_path_getimagesize_failed', __( 'Failed to get $file_path information using "@getimagesize".','saha'), $file_path );
			...
			if ( IMAGETYPE_GIF == $orig_type ) {
				if ( !imagegif( $new_image, $dest_file_name ) )
					return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (GIF)','saha' ) );
			}
			elseif ( IMAGETYPE_PNG == $orig_type ) {
				if ( !imagepng( $new_image, $dest_file_name ) )
					return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (PNG).','saha' ) );
			}
			else {
				...
				if ( !imagejpeg( $new_image, $dest_file_name, apply_filters( 'resize_jpeg_quality', 90 ) ) )
					return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (JPG).','saha' ) );
			...
			if ( !$new_size )
				return new WP_Error( 'resize_path_getimagesize_failed', __( 'Failed to get $dest_file_name (resized image) info via @getimagesize','saha' ), $dest_file_name );
 ...
 */
add_action( 'delete_attachment', 'saha_delete_resized_images' );
function saha_delete_resized_images( $post_id ) {
